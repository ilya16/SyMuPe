...
class SequenceCollator(DataCollator):
    ...
    @staticmethod
    def get_lengths(batch: Sequence[SequenceSample], attr: str, condition_fn: callable = None):
        if condition_fn is not None:
            lengths = [len(getattr(sample, attr)) * condition_fn(sample) for sample in batch]
        else:
            lengths = [len(getattr(sample, attr)) for sample in batch]
    ...
    def get_max_lengths(self, batch: Sequence[SequenceSample], inference: bool = False):
        lens_seq = self.get_lengths(batch, "seq")
        max_lens = {"sequence": np.max(lens_seq) if inference else self.pad_len(np.max(lens_seq))}
        if batch[0].task_seq is not None:
            lens_task = self.get_lengths(batch, "task_seq")
            max_lens["task_sequence"] = np.max(lens_task)
        return max_lens

    def _init_seq_data(self, batch_size: int, max_len: int, compound_factor: int = 0):
        seq_shape = (
            (batch_size, max_len, compound_factor) if compound_factor > 0 else (batch_size, max_len)
        )
        seq_data = SeqInputs(
    ...
    def init_data(self, batch: Sequence[SequenceSample], inference: bool = False):
        ...
        return SequenceInputs(
            sequences=self._init_seq_data(
                b, max_lens["sequence"], compound_factor=sample.seq.ids.shape[-1]
            ),
            segments=(
                SeqSegments(
            ...
            pedals=torch.full((b, max_lens["sequence"], 2), fill_value=-1.0),
            ...
            task_sequences=(
                self._init_seq_data(
                    b, max_lens["task_sequence"], compound_factor=sample.task_seq.ids.shape[-1]
                )
            ...
            score_sequences=(
                self._init_seq_data(
                    b, max_lens["sequence"], compound_factor=sample.score_seq.ids.shape[-1]
                )
                if sample.score_seq is not None
                else None
            ),
            context_sequences=(
                self._init_seq_data(
                    b, max_lens["sequence"], compound_factor=sample.context_seq.ids.shape[-1]
                )
                if sample.context_seq is not None
                else None
            ),
            emotion_labels=(
                torch.zeros(b, dtype=torch.long) if sample.emotion_labels is not None else None
            ),
            emotion_embeddings=(
                torch.zeros(b, max_lens["sequence"], sample.emotion_embeddings.shape[-1])
    ...
    @staticmethod
    def _process_sequence(i: int, seq: TokSequence, seq_data: SeqInputs):
        seq_len = len(seq)
    ...
    def _process_auxiliary(self, i: int, sample: SequenceSample, data: SequenceInputs):
        ...
        if hasattr(data, "pedals") and sample.pedals is not None:
            # in most cases number of pedals is less than the number of notes
            pedals = sample.pedals[:seq_len]
    ...
    def process_sample(
        self, i: int, sample: SequenceSample, data: SequenceInputs, inference: bool = False
    ):
        # process sequence
...
def sample_mask(
    mask: torch.Tensor, prob: float | list[float] | torch.Tensor, level: MaskLevel | None = None
):
    ...
    if level == MaskLevel.START:
        ...
        mask_range = torch.maximum(torch.tensor(1.0), (num_tokens - num_mask_tokens).flatten())
...
class LMSequenceCollator(SequenceCollator):
    def __init__(
        ...
        context_known_dropout: float = 0.0,
        context_label_dropout: float = 0.0,
    ):
        ...
        mask_levels = (
            list(mask_level) if isinstance(mask_level, (dict, DictConfig)) else [mask_level]
        )
        ...
        assert self.replace_prob + self.random_token_prob <= 1.0
    ...
    def _mask_sequence(
        ...
        if (
            self.mask_token_dims is not None
            and len(self.mask_token_dims) > 0
        ...
        if mask_level in LAST_NOTES_MASKS:
            ...
            replace_prob, random_token_prob = 1.0, 0.0
        ...
        if random_token_prob > 0:
            ...
            if random_seq is None:
                assert num_tokens is not None, (
                    "`num_tokens` must be provided for computing random tokens"
                )
                if seq.ndim == 3:
        ...
        if self.mask_token_dims_by_type_id is not None and type_ids is not None:
            for type_id, mask_token_dims in self.mask_token_dims_by_type_id.items():
                ...
                type_id_mask = (
                    type_id_dim_mask if type_id_mask is None else type_id_mask | type_id_dim_mask
                )

            masked_seq.masked_fill_(type_id_mask, self.mask_token_id)
            if masked_values is not None:
                masked_values.masked_fill_(
                    type_id_mask, self.label_pad_token_id - self.mask_token_id
                )

        # derive labels to predict
        if self.copy_sequence_prob > 0:
            copy_mask = prob_mask_like(
                seq[:, 0, 0] if seq.ndim == 3 else seq[:, 0], self.copy_sequence_prob
            )
        ...
        if self.mask_level_exceptions is None:
            ...
            for task_idx in task_ids:
                ...
                for mask_level, tasks in self.mask_level_exceptions.items():
                    if task in tasks:
                        _mask_levels[mask_level] = 0.0

                probs = list(_mask_levels.values())
                mask_level_ids.append(
                    random.choices(list(range(len(probs))), weights=probs, k=1)[0]
                )
    ...
    def dropout_context(
        ...
        # predicted notes
        label_note_mask = torch.any(label_mask, dim=-1) if label_mask.ndim == 3 else label_mask

        def compute_dropout_mask(tensor):
            mask = torch.zeros_like(tensor, dtype=torch.bool)

            if self.context_known_dropout > 0:  # dropout context for known notes
                drop_context_mask = prob_mask_like(
                    sequences.tokens[:, 0, 0], self.context_known_dropout
                )[:, None]
                mask = mask | (~label_note_mask & drop_context_mask)[..., None]
            if self.context_label_dropout > 0:  # dropout context for predicted notes
                drop_label_mask = prob_mask_like(
                    sequences.tokens[:, 0, 0], self.context_label_dropout
                )[:, None]
        ...
        if emotion_embeddings is not None:
            ...
            emotion_embeddings[drop_mask] = 0.0
    ...
    def __call__(
        ...
        full_labels = SeqInputs(
            tokens=data.sequences.tokens, values=data.sequences.values, mask=data.sequences.mask
        )
...
class LMMultiSequenceCollator(SequenceCollator):
    ...
    def get_max_lengths(self, batch: Sequence[SequenceSample], inference: bool = False):
        lens_seq = self.get_lengths(
            batch,
            "seq",
            condition_fn=lambda sample: (
                sample.meta.encoding_type[0] != sample.meta.encoding_type[1]
            ),
        )
        lens_target_seq = self.get_lengths(batch, "target_seq")
        lens = lens_seq + lens_target_seq

        lens_enc = self.get_lengths(
            batch,
            "enc_seq",
            condition_fn=lambda sample: (
                sample.meta.encoding_type[0] != sample.meta.encoding_type[1]
    ...
    def _process_multi_sequence(
        self, i: int, sample: SequenceSample, seq_data: SeqInputs, seq_mask: torch.Tensor
    ):
        def _process_sequence_and_encoding(seq: TokSequence, enc_seq: TokSequence, cur_len: int):
            if self.task_sequences:
                ...
                if enc_seq.values is not None:
                    seq_data.values[i, cur_len : cur_len + len(enc_seq)] = torch.from_numpy(
                        enc_seq.values
                    )
    ...
    def process_sample(
        self, i: int, sample: SequenceSample, data: MultiSequenceInputs, inference: bool = False
    ):
        # process sequence
        self._process_multi_sequence(
            i, sample=sample, seq_data=data.sequences, seq_mask=data.sequence_mask
        )
...
class MixedLMSequenceCollator(SequenceCollator):
    ...
    def mask_sequence(self, seq: torch.Tensor, values: torch.Tensor | None):
        ...
        masked_values = None
        if values is not None:
            masked_values = values.clone().detach()
            masked_values = masked_values.masked_fill(
