...
class TokenSequenceBarIndexer(TokenSequenceIndexer):
    ...
    def compute_bar_indices(self, seq: TokSequence) -> np.ndarray:
        tokens = seq.ids
        bar_index = self.tokenizer.vocab_types_idx["Bar"]

        min_bar = tokens[0, bar_index] - self.tokenizer.zero_token
        total_bars = tokens[-1, bar_index] - self.tokenizer.zero_token + 1

        bar_diff = np.concatenate([[min_bar], np.diff(tokens[:, bar_index])])
        bar_changes = np.where(bar_diff > 0)[0]

        bars = np.concatenate([[0], np.cumsum(bar_diff[bar_changes]), [total_bars]])
        bar_changes = np.concatenate([[0], bar_changes, [tokens.shape[0]]])

        bar_indices = np.full(bars[-1] + 1, -1, dtype=np.int32)
