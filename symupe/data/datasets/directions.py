...
def get_direction_performances_map(
    ...
    for score_idx, score in enumerate(sp_dataset.score_names):
        ...
        if key not in score_dir_map:
            score_seq = sp_dataset.scores[score_idx]
            if level == "bar":
                dir_map = np.zeros(score_seq.ids[-1, 0] - sp_dataset.tokenizer.zero_token + 1)
            else:
                dir_map = np.zeros(len(score_seq))
        else:
...
class DirectionBarEmbeddingDataset(Dataset):
    ...
    def get_emb_by_idx(self, emb_idx: int) -> torch.Tensor:
    ...
    def __getitem__(self, idx) -> tuple[int, torch.Tensor, int]:
