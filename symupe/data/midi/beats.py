    ...
    *,
    ...
    for i, time_sig in enumerate(time_sigs):
        ...
        inter_beat_interval = get_inter_beat_interval(
            time_sig=time_sig, ticks_per_bar=ticks_per_bar, ticks_per_quarter=ticks_per_quarter
        )
...
def get_performance_beats(
    ...
    for i, beat in enumerate(score_beats):
        ...
        if onset_ticks[onset_idx] == beat:
            ...
            # interpolate
            left = -1
            if onset_ticks[0] < beat:
                left = np.searchsorted(onset_ticks, beat, side="left") - 1

            if left < 0:
                left = len(onset_ticks)
                if onset_ticks[-1] > beat:
                    left = np.searchsorted(onset_ticks, beat, side="right")

            right = max(i + 1, left + 1)
            while right < len(onset_ticks) and onset_ticks[right] < beat:
                right += 1

            if right == len(onset_ticks):
                left, right = left - 1, left
                while left > 0 and onset_ticks[left] > beat:
                    left -= 1
