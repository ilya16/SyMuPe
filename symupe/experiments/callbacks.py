...
from ..modules.constructor import Registry, VariableModuleConfig, Constructor
...
class TrainerCallback(Constructor):
    """
...
CallbacksRegistry = type("_VocoderCallbacksRegistry", (Registry,), {})()


@dataclass
class CallbackConfig(VariableModuleConfig):
    _target_: str


@dataclass
class DefaultFlowCallbackConfig(VariableModuleConfig):
    _target_: str = "default_flow"


@CallbacksRegistry.register("default_flow")
...
@dataclass
class ProgressCallbackConfig(VariableModuleConfig):
    _target_: str = "progress"


@CallbacksRegistry.register("progress")
class ProgressCallback(TrainerCallback):
    ...
    def on_epoch_begin(self, config: TrainerConfig, state: TrainerState, control: TrainerControl, **kwargs):
        if state.is_local_main_process:
            if control.is_train:
                ...
                self.evaluation_bar = tqdm(total=kwargs.get("steps_in_epoch", None), leave=False)
    ...
    def on_epoch_end(self, config, state, control, **kwargs):
        if state.is_local_main_process:
            ...
            step_message = f"epoch: {int(state.epoch):3d}/{state.num_train_epochs:3d} (step: {state.global_step})"
...
@dataclass
class PrinterCallbackConfig(VariableModuleConfig):
    _target_: str = "printer"


@CallbacksRegistry.register("printer")
...
@dataclass
class EpochReproducibilityCallbackConfig(VariableModuleConfig):
    _target_: str = "epoch_seed"


@CallbacksRegistry.register("epoch_seed")
...
@dataclass
class TrackerCallbackConfig(VariableModuleConfig):
    _target_: str = "tracker"
    accelerator: Accelerator = MISSING


@CallbacksRegistry.register("tracker")
class TrackerCallback(TrainerCallback):
    """
    A [`TrainerCallback`] that tracks the logs to the connected dashboards (tensorboard, wandb).
    Args:
        accelerator (`Accelerator`):
            The Accelerator instance with connected through `log_with` dashboards.
    """
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        eval_logs = kwargs.get("eval_logs", None)
        if eval_logs is not None:
            self.accelerator.log(eval_logs, step=state.global_step)
