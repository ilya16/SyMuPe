        ...
        if out.logits is not None and labels is not None:
            ...
            if self.distance_loss:
                ...
                dist_losses = {
                    ...
                    for i, (key, logits) in enumerate(out.logits.items())
                    if self.model.token_emb.embs[key].token_values is not None
                    and torch.any(_label_mask[..., i])
                }
        ...
        if out.values is not None and targets is not None:
            ...
            for i, key in enumerate(self.model.token_emb.embs.keys()):
                ...
                reg_losses[f"{key}/l1"] = masked_batch_mean(
                    F.l1_loss(out.values[key], targets[..., i], reduction="none"),
                    mask=target_mask[..., i],
                )
        ...
        def _unmask_step():
            ...
            if self.model.lm_head is not None:
                logits_i, pred_tokens_i, pred_values_i = self.model.lm_head.infer(
                    outputs.hidden_state[:, idx],
                    keys=self.token_keys,
                    temperature=temperature,
                    top_k=top_k,
                    top_p=top_p,
                    num_first_mask=self.num_special_tokens,
                    filter_key_ids=filter_key_ids,
                    ignore_non_special=ignore_non_special,
                    tokenizer=tokenizer,
                )
        ...
        if single_run:
            ...
            if outputs.logits is not None:
                ...
                pred_tokens = torch.cat(
                    list(
                        map(
                            lambda l_i: filter_and_sample(
                                l_i, temperature=temperature, top_k=top_k, top_p=top_p
                            ),
                ...
                if out_values is not None:
                    ...
                    pred_values = tokenizer.decode_values(
                        ...
                        token_type=list(outputs.logits.keys()),
                        normalize=True,
                    )

                    pred_values = pred_values.view(*shape, pred_tokens.shape[-1]).float()
...
class TupleTransformerARWrapper(TupleTransformerLMWrapper):
        ...
        for idx in pbar:
            ...
            if self.model.lm_head is not None:
                ...
                if fix_errors and tokenizer is not None:
                    ...
                    fill_token_id = (
                        pitch.item() if pitch <= eod_token_id else tokenizer.ignore_token
                    )
                    ...
                    for i, (key, key_logits) in enumerate(logits_i.items()):
                        if key.startswith("Pitch"):
                            ...
                            pred_tokens_i[..., i] = torch.full_like(
                                pred_tokens_i[..., i], fill_value=fill_token_id
                            )

                if (fix_errors and tokenizer is not None) or known_tokens is not None:
                    token_types = list(logits_i.keys())
                    pred_values_i = tokenizer.decode_values(
                        pred_tokens_i, token_type=token_types, normalize=True
                    ).float()
            ...
            if known_tokens is not None:
                ...
                if self.token_indices is not None:
                    ...
                    pred_tokens_i_full[..., ~known_mask[0, idx]] = pred_tokens_i[
                        ..., ~known_mask[0, idx]
                    ]
            ...
            if known_values is not None:
                ...
                if self.token_indices is not None:
                    ...
                    pred_values_i_full[..., ~known_mask[0, idx]] = pred_values_i[
                        ..., ~known_mask[0, idx]
                    ]
            ...
            out_values = (
                torch.cat((out_values, pred_values_i), dim=1) if out_values is not None else None
            )
            mask = F.pad(mask, (0, 1), value=True)

            eos_token_id = eod_token_id or self.eos_token_id
            if eos_token_id is not None:  # eos only in `Bar`
                is_eos_tokens = (out_tokens[..., -1, 0] == eos_token_id) | (
                    out_tokens[..., -1, 0] == self.eos_token_id
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        context_tokens, context_values = (
            kwargs.get("context_tokens", None),
            kwargs.get("context_values", None),
        )
        ...
        score_tokens, score_values = (
            kwargs.get("score_tokens", None),
            kwargs.get("score_values", None),
        )
        ...
        **kwargs,
        ...
        def _unmask_step():
            ...
            outputs = self(
                tokens=token_inputs,
                values=value_inputs,
                masked_tokens=masked_token_inputs,
                masked_values=masked_value_inputs,
                mask=mask[:, : idx + 1],
                return_embeddings=True,
                return_cache=True,
                **kwargs,
