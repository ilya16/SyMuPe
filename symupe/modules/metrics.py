...
def masked_batch_mean(
    ...
    if ignore_empty:
        tensor = num[den != 0.0] / den[den != 0.0]
    else:
...
def weighted_distance(
    ...
    values = (
        (targets[..., None] - token_values.to(targets.device)[None, None, :]).abs() * probs
    ).sum(dim=-1)
    return reduce_metrics(values, mask, reduction)
...
def mean_reciprocal_rank(
    ...
    ranks = (
        torch.arange(1, sorted_indices.shape[1] + 1, device=sorted_indices.device)
        .unsqueeze(0)
        .expand(sorted_indices.shape[0], -1)
    )

    # replace non-matching positions with a large number so that min() finds the correct rank
    rr = torch.where(
        sorted_indices == labels[:, None], ranks, torch.tensor(1e5, device=ranks.device)
    )
