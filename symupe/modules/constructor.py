...
class Constructor:
    @classmethod
    def _pre_init(cls, config: DictConfig | ModuleConfig | None = None, **parameters):
        def is_module_parameter(param):
    ...
    @classmethod
    def init(cls, config: DictConfig | ModuleConfig | None = None, **parameters):
...
def merge(*containers: DictConfig | ModuleConfig | dict, as_omega=False) -> dict | DictConfig:
    readonly = False
    _containers = []
...
class Registry:
    ...
    def instantiate(self, config: VariableModuleConfig | DictConfig, **kwargs):
        module: Constructor = self.get(config.get("_target_", next(iter(self._objects))))
