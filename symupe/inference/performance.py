...
def perform_score(
    ...
    if isinstance(generator, CFMMusicTransformerGenerator):
        ...
        generator_kwargs = {
            ...
            "top_p": mlm_top_p,
            "filter_key_ids": {
                key: list(range(tokenizer.zero_token))
                for key in generator.used_token_types
            }
        }
        ...
        raw_gen_seq = tokenizer.decompress(
            tokenizer.denormalize_values(
                replace(
                    ...
                    ids=gen_tokens[i], values=gen_values[i],
...
def postprocess_performance_sequence(tokenizer: SyMuPe, gen_seq: TokSequence, score_seq: TokSequence):
    vocab = gen_seq.vocab
    ...
    if "Sustained" in vocab and not tokenizer.has_token_types(gen_seq, "Sustained"):
        if "TimeDurationSustain" in vocab and "TimeDuration" in vocab:
            sustained = gen_seq.values[:, vocab["TimeDurationSustain"]] > gen_seq.values[:, vocab["TimeDuration"]]
...
def performance_sequence_to_midi(generator: Generator, gen_seq: TokSequence):
    ...
    if "TimeDurationSustain" in gen_seq.vocab:
        ...
        td_idx, tdsus_idx = gen_seq_sus.vocab["TimeDuration"], gen_seq_sus.vocab["TimeDurationSustain"]
